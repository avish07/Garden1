...
public class ReadMoreTextView: UITextView {
    ...
    var textStyle: TextStyle = TextStyle(_font: TextStyles.body2Regular.font , _lineHeight: TextStyles.body2Regular.lineHeight, _color: AppColors.textPrimary) {
        didSet {
            addTextAccordingToStyle()
        }
    }
    
    var readMoreTextStyle: TextStyle = TextStyle(_font: TextStyles.body2Medium.font , _lineHeight: TextStyles.body2Medium.lineHeight, _color: AppColors.fillPrimary) {
        didSet {
            addReadMoreButton()
        }
    }
     ...
     func addTextAccordingToStyle() {
        self.attributedText = textStyle.applyToString(string: text ?? "")
    }
     ...
     */
    @IBInspectable
    public var maximumNumberOfLines: Int = 6 {
        didSet {
    ...
    public override var text: String! {
        didSet {
            if let text = text {
                _originalAttributedText = textStyle.applyToString(string: text ?? "")
                self.attributedText = textStyle.applyToString(string: text ?? "")
                addReadMoreButton()
            } else {
    ...
    public override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        if let point = touches.first?.location(in: self) {
            guard pointIsInReadMoreOrReadLessTextRange(point: point) != nil else { return }
            handler?()
        }
    ...
    func addReadMoreButton() {
        ...
        var elepsisString = NSMutableAttributedString(attributedString:   readMoreTextStyle.applyToString(string: "..."))
        
        var readMoreString =  readMoreTextStyle.applyToString(string: "read more")
        
        elepsisString.append(readMoreString)
        self.attributedReadMoreText = elepsisString
    }
